...
public interface TcnudRepository extends JpaRepository<Tcnud, CompositePK> {
    ...
    List<Tcnud> getDataByStock(String Stock);
    ...
    @Query(value = "SELECT * FROM tcnud WHERE Stock = ?1 AND BranchNo = ?2 AND CustSeq = ?3 ORDER BY Stock", nativeQuery = true)
    List<Tcnud> getDataByStockAndBranchAndCustSeq(String Stock, String BranchNo, String CustSeq);

    @Query(value = "SELECT * FROM tcnud WHERE BranchNo = ?1 AND CustSeq = ?2 ORDER BY Stock", nativeQuery = true)
